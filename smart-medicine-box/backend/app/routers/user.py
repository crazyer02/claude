...
from app.models import get_db, User, FamilyBinding
...
async def login(request: UserLoginRequest, db: Session = Depends(get_db)):
    ...
    try:
        result = await UserService.login(db, request.code)
...
async def update_profile(
    ...
    user = UserService.update_profile(db, current_user, request)
...
async def bind_family(
    ...
    """子女绑定老人用户，以便查看老人用药情况"""
    try:
        binding = UserService.bind_family(db, current_user, request)
...
async def get_family_members(
    ...
    """获取某位老人的所有绑定家人"""
    return UserService.get_family_members(db, elderly_user_id)
...
async def get_binded_elderly(
    ...
    return UserService.get_binded_elderly(db, current_user.id)
...
async def get_elderly_info(
    ...
    """子女查看指定老人的用药详情"""
    try:
        return UserService.get_elderly_info(db, elderly_id, current_user.id)
    except ValueError as e:
